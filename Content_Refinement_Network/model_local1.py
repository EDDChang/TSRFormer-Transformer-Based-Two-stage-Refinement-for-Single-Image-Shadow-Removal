...
'''
Use this model to replace the original one:
from model_local import ShadowFormer_base
model = ShadowFormer_base()
model.load_state(...)
'''

train_size=(1,3,320,320) # Please check the crop size during training
...
class Local_Base():
    def convert(self, *args, **kwargs):
        # print(args, kwargs)
        train_size = kwargs['base_size']
        replace_layers(self, *args, **kwargs)
        # imgs = torch.rand(train_size)
        # mask = torch.rand(train_size)
        # with torch.no_grad():
        #     self.forward(imgs, mask)

class ShadowFormer_base(Local_Base, ShadowFormer):
    def __init__(self, *args, base_size=320, fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        ShadowFormer.__init__(self, *args, **kwargs)
